...
python -m proton.cli examples/hello.pt --to-py build/hello_runner.py
python -m proton.cli examples/hello.pt --to-exe proton_hello   # PyInstaller varsa
```
...
- Yeni yardımcı komutlar eklendi: `help>`, `calc>`, `retry>`, `debug>`, `trace>`, `profile>`.
...
## PT -> EXE / Runner Dönüşümü
- `--to-py`: Proton scriptini self-runner bir Python dosyasına dönüştürür.
- `--to-exe`: Aynı runner üzerinden PyInstaller ile tek dosya executable üretir.
- Runtime içinden de `to.py>` ve `to.exe>` komutları çağrılabilir.

Örnek Proton:
```proton
to.py> ("examples/hello.pt", "build/hello_runner.py")
```
