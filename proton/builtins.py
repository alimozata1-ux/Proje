...
def _resolve_path_arg(raw: str, ctx) -> Path:
    try:
        value = _eval(raw, ctx)
    except Exception:  # noqa: BLE001
        value = raw
    return _ensure_path(value, ctx)
...
def register_builtin_commands(registry) -> None:
    ...
    registry.register("help>", lambda a, c: sorted(registry.handlers.keys()))

    # Matematik
    registry.register("+:", lambda a, c: sum(_eval(a, c)))
    registry.register("-:", lambda a, c: _eval(a, c)[0] - _eval(a, c)[1])
    registry.register("*:", lambda a, c: math.prod(_eval(a, c)))
    registry.register("/:", lambda a, c: _eval(a, c)[0] / _eval(a, c)[1])
    registry.register("^:", lambda a, c: _eval(a, c)[0] ** _eval(a, c)[1])
    registry.register("%:", lambda a, c: _eval(a, c)[0] % _eval(a, c)[1])
    ...
    # Dosya
    registry.register("mk>", lambda a, c: _resolve_path_arg(a, c).touch())
    registry.register("del>", lambda a, c: _resolve_path_arg(a, c).unlink(missing_ok=True))
    registry.register("dir>", lambda a, c: os.listdir(_resolve_path_arg(a or ".", c)))
    registry.register("r<", lambda a, c: _resolve_path_arg(a, c).read_text(encoding="utf-8"))

    def write_file(a: str, c):
        path_expr, data_expr = _eval(a, c)
        path = _ensure_path(path_expr, c)
        path.write_text(str(data_expr), encoding="utf-8")
        return str(path)
    ...
    def copy_file(a: str, c):
        src, dst = _eval(a, c)
        src_p = _ensure_path(src, c)
        dst_p = _ensure_path(dst, c)
        return shutil.copy(src_p, dst_p)

    def move_file(a: str, c):
        src, dst = _eval(a, c)
        src_p = _ensure_path(src, c)
        dst_p = _ensure_path(dst, c)
        return shutil.move(src_p, dst_p)

    registry.register("w>", write_file)
    registry.register("append>", append_file)
    registry.register("copy>", copy_file)
    registry.register("move>", move_file)
    registry.register("size>", lambda a, c: _resolve_path_arg(a, c).stat().st_size)
    registry.register("perm>", lambda a, c: oct(_resolve_path_arg(a, c).stat().st_mode))
    ...
    registry.register("sh:", shell)
    registry.register("sh>", shell)
    registry.register("sys:", shell)
    ...
    registry.register("sign>", lambda a, c: hmac.new(str(_eval(a, c)[0]).encode(), str(_eval(a, c)[1]).encode(), hashlib.sha256).hexdigest())
    ...
    def timer(args: str, ctx):
        ...
        return {"value": value, "elapsed_ms": round((time.perf_counter() - start) * 1000, 3)}
    ...
    def json_cmd(a: str, c):
        raw = a.strip()
        if raw.startswith("{") or raw.startswith("["):
            return json.loads(raw)
        return json.dumps(_eval(a, c), ensure_ascii=False)

    registry.register("json>", json_cmd)
    ...
    def chunk_cmd(a: str, c):
        seq, size = _eval(a, c)
        return [seq[i : i + size] for i in range(0, len(seq), size)]

    registry.register("chunk>", chunk_cmd)
    ...
    def to_py(args: str, ctx):
        pt_path, out_path = _eval(args, ctx)
        return str(build_runner(pt_path, out_path, sandbox=ctx.sandbox.enabled))

    def to_exe(args: str, ctx):
        pt_path, out_name = _eval(args, ctx)
        return str(build_executable(pt_path, out_name, sandbox=ctx.sandbox.enabled))
    ...
    registry.register("to.py>", to_py)
    registry.register("to.exe>", to_exe)
    registry.register("retry>", lambda a, c: _retry(_eval(a, c)))

    # Şimdilik stub bırakılanlar
    for name in [
        ...
        "doc>", "test>", "build>", "match:",
        "if:", "elif:", "else:", "for:", "while:", "break>", "cont>", "f->", "async->",
    ]:
